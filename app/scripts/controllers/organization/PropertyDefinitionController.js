(function(module) {
	mifosX.controllers = _.extend(module, {
		PropertyDefinitionController : function(scope, location,  $modal, route, webStorage,resourceFactory,PermissionService,$upload,$rootScope,API_VERSION,paginatorService) {
	      	...
	      	function  approve1($scope, $modalInstance) {
	      		...
	      		$scope.approve = function () {
	      			 $upload.upload({/*41.75.85.206:8080*/
	                     url:'https://192.168.1.201:8443/obsplatform/api/v1/propertycodemaster/documents',
	                     //$rootScope.hostUrl+ API_VERSION +'/propertycodemaster/documents', 
	                     data: scope.formData,
	                     file: $scope.file
	                   }).then(function(data) {
	...
	mifosX.ng.application.controller('PropertyDefinitionController',[ 
	    '$scope',
	    '$location',
	    '$modal',
	    '$route',
	    ...
	    'API_VERSION',
	    'PaginatorService',
	    mifosX.controllers.PropertyDefinitionController 
	    ]).run(function($log) {
	    	$log.info("PropertyDefinitionController initialized");
	    });
}(mifosX.controllers || {}));
