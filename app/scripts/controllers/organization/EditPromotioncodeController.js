(function(module) {
	mifosX.controllers = _.extend(module, {
		EditPromotioncodeController : function(scope, resourceFactory,
				location, routeParams, dateFilter, $rootScope) {
			...
			resourceFactory.promotionResource.getPrmotioncodeDetails({
				promotioncodeId : routeParams.id
			}, function(data) {
			...
			scope.submit = function() {
				...
				var startdate = dateFilter(scope.start.startDate,
						'dd MMMM yyyy');

				this.formData.startDate = startdate;

				resourceFactory.promotionResource.update({
					promotioncodeId : routeParams.id
	...
	mifosX.ng.application.controller(
			'EditPromotioncodeController',
			[ '$scope', 'ResourceFactory', '$location', '$routeParams',
					'dateFilter', '$rootScope',
					mifosX.controllers.EditPromotioncodeController ]).run(
			function($log) {
