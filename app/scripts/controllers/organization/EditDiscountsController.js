(function(module) {
	mifosX.controllers = _.extend(module, {
		EditDiscountsController : function(scope, resourceFactory, location,
			...
			scope.start = {};
			scope.date = {};
			resourceFactory.discountsResource.getDiscountDetails({
				discountId : routeParams.id
					...
					function(data) {
						...
						scope.formData = data;
						var actDate = dateFilter(data.discountStartDate,
								'dd MMMM yyyy');
			...
			scope.submit = function() {
				...
				this.formData.startDate = dateFilter(scope.start.startDate,
						'dd MMMM yyyy');
				...
				resourceFactory.discountsResource.update({
					discountId : routeParams.id
	...
	mifosX.ng.application.controller(
			'EditDiscountsController',
			[ '$scope', 'ResourceFactory', '$location', '$routeParams',
					'dateFilter', '$rootScope',
					mifosX.controllers.EditDiscountsController ]).run(
			function($log) {
