(function(module) {
	mifosX.controllers = _.extend(module, {
		CreateCurrencyDetailsController : function(scope, resourceFactory,
				location, $rootScope) {
			scope.countryDatas = [];
			scope.currencydatas = [];
			...
			scope.submit = function() {
				...
				resourceFactory.currencyResource.save(this.formData, function(
						data) {
	...
	mifosX.ng.application.controller(
			'CreateCurrencyDetailsController',
			[ '$scope', 'ResourceFactory', '$location', '$rootScope',
					mifosX.controllers.CreateCurrencyDetailsController ]).run(
			function($log) {
