		...
		// *********************** InVenture controller ***********************
	  		scope.fetchInventureScore = function(){
		        ...
		        /** Delete Popup Controller */
		 		...
		 		scope.deviceSwapPopupFun = function(serialNum){
		 			$modal.open({
		 		...
		 		function DeviceswapPopupController($scope, $modalInstance,serialNum){
		 			
		 			$scope.configDeviceAgrement = webStorage.get('client_configuration').deviceAgrementType;
		 			 
		 			resourceFactory.deviceSwapAssociationResource.get({clientId: scope.clientId,serialNum:serialNum} , function(data) {
		                $scope.association = data;  
		                $scope.officeId = data.clientId;
		 			...
		 			$scope.submit = function(serialNumber){
		 				$scope.formData = {};
			            $scope.formData.provisionNum=serialNumber;
			            $scope.formData.serialNo=$scope.association.serialNum;
			            $scope.formData.deviceAgrementType=$scope.configDeviceAgrement;
			            $scope.formData.saleId=$scope.association.saleId;

			            resourceFactory.hardwareSwapResource.save({'clientId': scope.clientId},$scope.formData,function(data){
			            	$modalInstance.close('delete');
			                route.reload();
			              },function(errorData){
